...
static int bt_sock_name2bth(int devid, const char *btname, bdaddr_t * btaddr)
{
	...
	if ((file = fopen(cfname, "r+t")) == NULL && (file = fopen(cfname, "w+t")) == NULL)
		return -1;
	log(obex_outfile, 2, "Resolving name '%s' ...\n", btname);
	if ((niinf = hci_inquiry(devid, 32, -1, NULL, &piinf, 0)) < 0) {
		log(obex_outfile, 2, "hci_inquiry error\n");
		fclose(file);
		return -1;
	}
	if ((dd = hci_open_dev(devid)) < 0) {
		log(obex_outfile, 2, "Unable to open HCI device hci%d\n", devid);
		free(piinf);
		fclose(file);
		return -1;
	}
...
int bt_sock_open(int devid, bdaddr_t const *btaddr, int channel, int timeout)
{
	...
	if (connect(sock, (struct sockaddr *) &raddr, sizeof(raddr)) < 0) {
		...
		if (!FD_ISSET(sock, &wfds)) {
			log(obex_outfile, 2, "connect timeout\n");
			close(sock);
			return -1;
		}
	}
	...
	if (gt->portname != NULL) {
		if (force && gt->rfd != -1) {
			...
			if (ioctl(gt->rfd, TCSBRKP, 0) < 0)
				log(obex_outfile,1,"ERROR: Unable to send break!\n");
		}
	}
	if (gt->rfd != -1)
		close(gt->rfd);
...
struct cobex_context *cobex_open(int devid, const char *port, int timeout)
{
	...
	gt = (cobex_context*)malloc(sizeof(struct cobex_context));
	if (gt == NULL)
		return NULL;
	...
	if ((chan = strchr((const char *) port, '@')) != NULL) {
		...
		if (bt_sock_str2addr(gt->addr, &gt->btaddr) < 0 &&
			...
			log(obex_outfile, 2, "Unable to resolve '%s'\n", gt->addr);
			cobex_cleanup(gt, 0);
			return NULL;
		}
		gt->chan = atoi(chan + 1);
	...
	while (actual < length) {
		...
		if (frag < 0) {
			if (errno == EAGAIN) {
				...
				log(obex_outfile,1,"ERROR: Write error: %s\n", strerror(errno));
				return frag;
			}
		}
		actual += frag;
...
int cobex_handle_input(obex_t * handle, void *userdata, int timeout)
{
	...
	do {
		...
		if ((expectlen == 0) && (actual >= 3)) {
			...
			if ((unsigned int)expectlen > sizeof(gt->inputbuf)) {
